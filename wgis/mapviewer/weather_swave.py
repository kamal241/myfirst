...
def get_swave_data(lt1, lt2, ln1, ln2, t, hrsofst):
...
#        f_coll = {"type": "FeatureCollection", "features": features, "properties" : other_properties}
	...
	#	grbs.seek(0)
		...
		for shww in shwws[start:limit]:	
			shww_data, lats, lons = shww.data(lat1=lt1,lat2=lt2,lon1=ln1,lon2=ln2)
#			dt = datetime.datetime.strptime(shww.validDate,'%Y-%m-%d %H:%M:%S')
#			print dt
#			dtdt = datetime.datetime.strptime(shww.dataDate,"%Y%m%d")
#			dttm = shww.dataTime/10	
			dhr = int(shww.dataTime/100)
			tms = str(shww.dataDate) + "%02d" % dhr
			tmstmp = {"timestamp": tms, "hours": shww.validDate.hour - dhr}
			other_properties.append(tmstmp)
#			print shww.validDate,shww.validityDate,shww.validityTime,shww.dataDate,shww.dataTime 			
			shwws_all.append(shww_data)
		shwwd = np.dstack((shwws_all))
	
		mpwws = grbs.select(shortName="mpww")
		for mpww in mpwws[start:limit]:	
		...
		mpwwd = np.dstack((mpwws_all))
	        ...
	        for dirpw in dirpws[start:limit]:
	        ...
	        for perpw in perpws[start:limit]:
